...
import { saveRegistration, getRegistrations, isMockFirebase } from "./firebase/config";
...
export default function App() {
  ...
  const handleAdminLogin = (e) => {
    e.preventDefault();
    if (adminPassword === "rp2026") {
      setIsAdminLoggedIn(true);
      setAdminError("");
    } else {
      setAdminError("Invalid authorization code.");
    }
  };
              ...
              <div className="max-w-md mx-auto p-8 rounded-3xl bg-coal/20 border border-white/5 space-y-6 text-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-mineral-slate mt-2 tracking-wide font-light">
                    Enter the access pass-phrase specified in plan.md to inspect leads.
                  </p>
                </div>
                ...
                <form onSubmit={handleAdminLogin} className="space-y-4">
                  <div>
                    <input 
                      ...
                      className="w-full px-4 py-3 rounded-lg bg-coal border border-white/10 text-white placeholder:text-mineral-slate/50 text-center text-sm focus:outline-none focus:border-champagne-gold"
                    />
                    {adminError && <p className="text-red-400 text-xs mt-2 font-medium">{adminError}</p>}
                  </div>
                  <button 
                    type="submit"
                    className="w-full py-3 rounded-lg bg-white text-black font-semibold hover:bg-champagne-gold hover:text-black tracking-widest text-xs uppercase transition-all duration-300"
                  >
                    Authorize Access
                  </button>
                </form>
                <p className="text-[10px] text-mineral-slate font-mono">pass: rp2026</p>
              </div>
            ) : (
